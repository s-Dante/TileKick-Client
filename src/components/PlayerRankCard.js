...
export class PlayerRankCard extends Component {
    ...
    render() {
        ...
        let positionColor = 'text-text-dark';
        let borderColor = 'border-secondary-base';
        ...
        const styles = {
            card: `group relative bg-primary-base/80 rounded-2xl p-4 md:p-6 border ${borderColor} hover:bg-secondary-base transition-all duration-300 flex flex-col md:flex-row items-center gap-4 md:gap-6 overflow-visible mb-4`,
            posToken: `text-3xl md:text-5xl font-black ${positionColor} w-12 text-center flex-shrink-0`,
            imgWrapper: "relative flex-shrink-0",
            profileImg: `w-16 h-16 md:w-20 md:h-20 rounded-full border-2 ${isTop3 ? 'border-current ' + positionColor : 'border-secondary-base'} object-cover`,
            badge: "absolute -bottom-2 -right-2 bg-bg-base text-xs font-bold px-2 py-1 rounded-md border border-secondary-base shadow-sm flex items-center gap-1 z-10",
            infoBox: "flex-1 text-center md:text-left min-w-0",
            infoName: "text-xl font-bold text-text-base truncate",
            infoUser: "text-accent1-base text-sm truncate",
            statsBox: "flex flex-row md:flex-col gap-4 md:gap-1 text-sm md:border-l border-secondary-base md:pl-6",
            statRow: "flex items-center gap-2",
            statLabelW: "text-alert-success font-bold uppercase tracking-wider text-xs",
            statLabelL: "text-alert-error font-bold uppercase tracking-wider text-xs",
            statValue: "text-text-base font-medium",
            eloBox: "bg-bg-base/80 px-4 py-3 rounded-xl border border-secondary-base flex flex-col items-center justify-center min-w-[100px]",
            eloLabel: "text-text-dark text-xs uppercase tracking-widest font-bold mb-1",
            eloValue: `text-2xl font-black text-text-base ${isTop3 ? positionColor : ''}`,
